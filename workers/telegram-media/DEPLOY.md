...
| `MAX_UPLOAD_BYTES` | Variable | 否 | `20971520`（20MB）；**不建议上调**，见下 |

> Bot Token **不要**写进本 zip、Git、前端。

> **20 MB 是 `getFile` 的下载上限**，也就是本 worker 真正能读回来的上限。
> `sendDocument` 虽然收 50 MB，但 20–50 MB 的文件会上传成功、却永远读不回来
> （`/v1/media/:id` 走 `getFile` 失败 → 404），等于只写不可读。
> 1080p 长视频请改用 Site 储存（直连上游图站），不要走 TG。
